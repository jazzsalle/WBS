...
| IN-3 | **인건비 시트**: 행마다 `memberId`(숨김 열)로 인력을 잇는다. 인력당 그 연차의 기존 인건비 산출근거 행 수만큼 행(0건이면 빈 값 1행), 보이는 `세목` 열(부록 A.5 인건비 세목 라벨, 빈 행 기본 `내부인건비`) — 두 구간 참여는 행을 복사해 적는다 — 이름 매칭 없음(D-11~D-14를 쓰지 않는다). 양식에 없던 인력 행은 무시하지 않고 "알 수 없는 행"으로 미리보기에 남긴다. 참여율 0~100, 개월 0~12(연차 개월 초과는 경고), 축은 `현금`/`현물`. 금액 열은 **읽지 않는다**(수식 결과이며 PL-D7 — 서버가 다시 계산). |
| IN-4 | **사업비 시트**: 행마다 **`비목:세목` 복합 키**(숨김 열, 예 `activity:activity_meeting`·`consignment:default`)로 잇는다 — 세목 코드 `default`가 비목 6개에 공유되어 세목 코드만으로는 비목을 정할 수 없다. `_meta.subcategoryCodes`도 같은 복합 키다. 품명이 비고 단가·인자·조정액이 전부 비면 **빈 행**(무시). 단가·조정액은 원 단위 정수, 인자는 소수 허용. 세목 코드가 부록 A.5에 없으면 "알 수 없는 세목". **인자 열은 프리셋 `defaultFactors` 라벨 3개로 고정**이다 — 앱에서 라벨을 바꿔 둔 행(`시트(수량)` 등)은 왕복 후 프리셋 라벨로 돌아오고, `isPercent` 인자는 `/100`한 값으로 적혀 **금액만 보존**된다. 이 한계를 미리보기에 적는다. |
| IN-5 | 반영은 **양식에 행이 있는 비목 단위의 교체**다(D-15·D-15b — 기존 `commit_detail_import` RPC를 그대로 쓴다, 새 RPC 없음). 양식에 행이 하나도 없는 비목의 기존 산출근거는 **지우지 않고 유지**하며 미리보기에 "양식에 없는 비목 N행 유지"로 드러낸다. 반영 전 미리보기가 "추가 N · 변경 M · 삭제 K · 유지 U"와 합계·규칙 findings를 보여 주고, 스냅샷(I-17·D-17)을 남긴다. 추가/변경/삭제 판정은 두 시트의 숨김 `detailId` 열로 한다. |
