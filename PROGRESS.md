...
## Last updated
2026-08-10 (Phase 10 완료 — evaluator PASS. SOT v4.1)
...
## Next steps
1. **Phase 10 후속 정리 (블로킹 아님)**:
   - **stale `axisOverride` 교차 무효화**: Step 4에서 축을 바꾼 뒤 Step 2로 돌아가 같은 표에 `cashTotal`/`inKindTotal` role을 지정하면 그 행이 둘로 갈리며 `axisSuggested=false`가 되어, 서버가 매 미리보기마다 **명시 오류**로 막는다. 데이터 위험은 없고 role 변경을 되돌리면 풀리지만, 셀렉트가 사라진 상태라 화면에서 축 재지정을 지울 통로가 없다. `withColumnRoleOverride`가 관련 `axisOverrides`를 함께 비우면 해소된다
   - **Step 2 트리 숫자가 컬럼 role 재지정 후 갱신되지 않는다** — `inspectDetailSheet`가 draft를 받지 않기 때문이다(§9 시그니처). 실제 반영값은 Step 4가 정확히 낸다
   - `detail-wizard-state.ts`에 없어 `DetailImportWizard.tsx` 하단에 둔 순수 함수 3종(`withMemberDecision`·`withReplaceCategory`·`withRowDecision`)을 상태 모듈로 옮길지
   - `validateBlockChoices`의 D-9 거부 경로는 `'use server'` 파일 내부라 자동 테스트가 없다(순수 함수의 무시 동작만 단위 테스트로 고정)
   - Step 3이 명부를 `getTeam`으로 따로 읽는다 — 미리보기 페이로드에 실으면 왕복이 준다(§9 변경 필요)
2. **`npm run build` 전 `.next` 정리** — 잔여물이 있으면 standalone 복사 단계에서 ENOENT/프리렌더 실패가 산발한다. 클린 빌드는 19라우트 정상. 기존 "번들에 dev 잔여 정적 자산이 실려 나간다" 항목과 같은 뿌리로 보인다
1. **`created_by`가 끊긴 고아 행은 자동 청소가 되찾지 못한다 (설계상 한계)**:
   - `tests/global-setup.ts`는 이미 있고 동작도 옳다 — 2시간 이상 경과한 `wbs-test+%` 사용자를 찾아 **소유 행을 먼저 지우고 사용자를 나중에** 지운다(순서가 핵심)
   - 그런데 2026-08-09에 수동으로 치운 213개 중 **197개는 `created_by`가 NULL**이라 청소 대상에서 빠져 있었다. `projects.created_by`는 `app_users` 참조 `on delete set null`(N-8)이라, **누군가 사용자를 먼저 지우면 그 순간 소유자가 끊긴다.** `global-setup.ts` 9~11행 주석이 경고하는 바로 그 상황이다
   - 즉 장치를 안 만든 게 아니라, **이미 고아가 된 행은 구조적으로 되찾을 수 없다.** 앞으로 잔여물을 손으로 치울 때도 **반드시 소유 행 → 사용자 순서**를 지켜야 한다(2026-08-09 정리 SQL도 그 순서였다)
   - 보완한다면: 고아 판정을 소유자가 아닌 **다른 표식**(테스트 전용 과제명 접두사 등)으로도 걸 수 있게 하는 것. 지금은 이름이 자유 문자열이라 실데이터와 구분할 근거가 없다
2. **Tauri 후속 정리 2건 (블로킹 아님)**:
...
3. **남은 수동 검증** — 아래 "수동 검증 > 대기". **전체 개발이 끝난 뒤 한 번에 진행한다**(2026-08-08 결정). 인쇄 3종 A4 미리보기는 §12 검증표상 **원래부터 사람 몫**이다
4. **HR API 연동** (hr.unes.kr 직원 명부) — Phase 9에서 Member에 `annualSalary`가 생기므로 연동 지점이 여기 하나로 정해졌다. SOT 추가 후 별도 진행
5. **Phase 10 후보 (SOT §13에 등재)**: 제출 서식 엑셀 **내보내기**(§13 14번 — 산출근거가 앱에 쌓이면 실효성이 커진다), 산출근거 시트 **임포트**(§13 16번 — `lib/import/` 위에 세목·행 파서만 얹으면 된다), 부처별 고시율 프리셋(§13 17번 — PL-16에 따라 출처가 확실할 때만)
