...
## Last updated
2026-08-09 (Phase 10 SOT 작성 완료 — SOT v4.1. 구현 미착수)

## Current goal
**Phase 10(산출근거 시트 임포트) 구현.** SOT v4.1이 스펙을 확정했다. 다음 단계는 `/phase-run 10`.
...
## Next steps
0. **`/phase-run 10`** — 산출근거 시트 임포트 구현. SOT §6.11·§7.9.3·부록 B.8·C.2가 기준이다
   - **B.7 ↔ B.8 등가**를 반드시 통합 테스트로 고정한다 — 손으로 넣은 결과와 임포트한 결과가 같은 `budget_items`에 도달해야 한다
   - **PL-10a 유지**: `commit_detail_import` RPC에 금액 산식을 넣지 않는다
