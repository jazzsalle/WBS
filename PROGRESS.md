...
## Last updated
2026-08-07 (Phase 8 구현 완료 — v1 계획 Phase 전부 소진)

## Current goal
**v1 마감 처리 → 이후 Phase 9(예산 "제안" 모드) SOT 작성.** Phase 0~8의 계획된 범위는 전부 구현됐다.

> ⚠️ **v1 마감에 남은 것은 "코드"가 아니라 "확인"이다.** 아래 "수동 검증 > 대기"에 Phase 1·4·5·5.5·6·7·8치가 쌓여 있다. 2026-08-07에 고위험 5건은 사람이 직접 훑어 통과시켰지만(아래 완료 항목), 나머지는 여전히 evaluator 판정만 받은 상태다.
...
## Next steps
1. **Tauri 후속 정리 2건 (블로킹 아님)**:
   - **번들에 dev 잔여 정적 자산이 실려 나간다.** `src-tauri/resources/web/.next/static/`에 비압축 dev 청크(`chunks/app/layout.js` 3.67MB)와 `static/development/`·`static/webpack/`가 들어 있다. `prepare-sidecar.mjs`가 `.next/static`을 통째로 `cpSync` 하는데 이전 `next dev` 잔여물이 `next build`로 지워지지 않아 따라간다. 프로덕션 매니페스트가 참조하지 않는 죽은 파일이라 동작·보안 문제는 없지만 설치 파일이 커진다. 필터를 넣거나 빌드 전에 `.next`를 정리할 것
   - **배포는 NSIS setup.exe만 올린다** — SOT §14.1에 명문화했다. MSI도 함께 생성되지만 `perMachine`이라 관리자 권한을 요구한다(§12 위반). 공유 폴더에 둘 다 두면 사용자가 잘못된 쪽을 집는다
2. **남은 수동 검증** — 아래 "수동 검증 > 대기". 고위험 5건은 2026-08-07에 통과시켰고 나머지가 남았다. 인쇄 3종 A4 미리보기는 §12 검증표상 **원래부터 사람 몫**이다
3. **Phase 9 — 예산 "제안" 모드 (v1 마감 후, SOT 작성부터)**
   - 지금 연구비 화면은 "수행" 한쪽만이다: 확정 예산 기록 → 집행 누적 → 집행률. 엑셀은 **들여오는** 방향(§6.8)
   - "제안"은 데이터가 반대로 흐른다: 템플릿·단가·지침에서 예산을 **만들어내고** → 검증하고 → 제출 서식으로 **내보낸다**
   - **재사용 가능**: 비목 12종 + 부록 C 별칭 사전(부처별 지침 목록화의 씨앗), `CASH_INKIND_AXIS`(현물 비중 규칙에 필수), `lib/import/` 파싱 파이프라인(인건비 엑셀 업로드를 이 위에 얹는다 — 새 파서 불필요)
   - **구조적으로 빠진 것**: ① **산출근거(내역) 계층이 없다** — `budget_items`는 `unique(year_id, category)`라 연차×비목당 한 행·총액만 담는다 ② 인건비 산정(인원×참여율×연봉×개월) — SOT §13 미결정 1번 "참여연구원 참여율(%)"이 정확히 이 지점 ③ 부처별 지침(간접비율·학생인건비 비중·비목 한도) 테이블·규칙 없음 ④ 익스포트 없음
   - Phase 5.5에서 실측 워크북의 **"연차별 산출근거 시트(240~250행)"** 때문에 시트 추천을 행 수→행 비율로 바꿔야 했다(S-13). **그 산출근거 시트가 제안 모드의 원본 데이터**인데 지금은 "총괄표가 아닌 시트"로 걸러내고만 있다
   - **착수 전 SOT에 정해야 할 4건** (CLAUDE.md: 코드보다 SOT 먼저): ① 제안/수행을 한 화면의 모드로 둘지 별도 화면으로 둘지 ② 내역 계층의 스키마 ③ 부처 지침의 표현 방식(규칙 테이블 vs 코드) ④ **인건비 데이터가 DB에 저장되는지, 계산에만 쓰이고 총액만 남는지 — 보안사항이라 이게 제일 먼저 정해져야 한다**
   - 스키마 변경이 동반되는 별도 Phase다. Phase 8에 끼워 넣을 크기가 아니다
4. **HR API 연동** (hr.unes.kr 직원 명부) — Phase 9의 인건비 산정과 얽힌다. SOT 추가 후 별도 진행
