...
## Next steps
1. **⚠️ dev DB 잔여물 정리 (사람이 해야 함 — 자동 정리가 권한 제한으로 차단됐다)**:
   - `wbs-test+%` 패턴 auth 사용자 **16명**, 테스트 과제 **213개**(전부 2026-08-07 UTC 17:52~17:55 3분 창 = 중단된 실행 4회분). 하위 데이터(members·tasks·notes·todos·budget_details)는 전부 0건이고, 이름도 전부 테스트 픽스처(`… 테스트 과제`, `CRUD 놀이터`, `order 확인 A` 등)
   - **실데이터는 없다**: 2026-08-07 수동 검증용 시드 과제(`aaaa0000-…`)는 이미 삭제된 상태이고 승인 사용자는 `sangraedo@unes.co.kr` 1명뿐이다
   - 정리 SQL (직결):
     ```sql
     begin;
     update public.projects set pm_member_id = null, lead_org_id = null;  -- 순환 FK 해소
     delete from public.projects;
     delete from auth.users where email like 'wbs-test+%';
     commit;
     ```
   - **이걸 치우지 않으면 `npm run test:destructive`가 거부한다**(가드가 테스트 소유가 아닌 행으로 본다)
   - 근본 대책: 테스트 시작 시 오래된 `wbs-test+%` 잔여물을 먼저 청소하는 장치. 중단이 반복되면 계속 쌓인다
...
2. **산출근거 패널에 `비고` 열을 추가했다.** §7.9.2 컬럼 목록에는 없지만 `BudgetDetail.note`(§5.17)를 편집할 화면이 여기뿐이라 칸이 없으면 저장된 값에 손이 닿지 않는다. 불필요하면 뺀다
3. **dev DB 잔여물 정리가 자동으로 차단됐다** — Next steps 1번. 사람이 직접 SQL을 돌려야 한다
4. **`npm run test:destructive`(K-7 백업 복원 왕복)를 아직 돌리지 않았다.** 대상 테이블이 25 → 26종으로 늘고 백업 왕복 테스트에 산출근거 1행 검증이 추가됐는데, **사람이 명시적으로 부를 때만 실행하는 명령**이라 자동으로 돌리지 않았다. 위 1번 잔여물을 치운 뒤 한 번 돌려 확인할 것을 권한다
