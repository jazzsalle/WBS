...
| IN-4 | **사업비 시트**: 행마다 세목 코드(숨김 열)로 잇는다. 품명이 비고 단가·인자·조정액이 전부 비면 **빈 행**(무시). 단가·조정액은 원 단위 정수, 인자는 소수 허용. 세목 코드가 부록 A.5에 없으면 "알 수 없는 세목". **인자 열은 프리셋 `defaultFactors` 라벨 3개로 고정**이다 — 앱에서 라벨을 바꿔 둔 행(`시트(수량)` 등)은 왕복 후 프리셋 라벨로 돌아오고, `isPercent` 인자는 `/100`한 값으로 적혀 **금액만 보존**된다. 이 한계를 미리보기에 적는다. |
