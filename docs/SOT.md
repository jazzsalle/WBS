...
- **X-4 — 수식 셀은 건드리지 않는다**(사용자 결정): 앱이 계산한 값은 **데이터 셀에만** 쓰고 소계·합계는 템플릿의 수식이 다시 계산한다. **엑셀이 재계산한 값과 앱의 값이 일치해야 하며 그 일치를 테스트로 고정한다** — 어긋나면 앱과 서식 중 하나가 틀린 것이다. 서식의 검산 구조를 살려 두는 것이 목적이다.
...
| # | 규칙 |
|---|---|
| X-4 | **수식 셀을 건드리지 않는다.** 앱이 계산한 값은 **데이터 셀에만** 쓰고 소계·합계·비율은 템플릿의 수식이 다시 계산한다. <br>**엑셀이 재계산한 값과 앱의 값이 일치해야 하며, 그 일치를 테스트로 고정한다** — 어긋나면 앱과 서식 중 하나가 틀린 것이고 어느 쪽인지 알아야 한다. 값을 전부 박아 넣으면 이 검산이 사라지고, 사용자가 엑셀에서 한 칸을 고쳐도 소계가 따라오지 않는다. |
