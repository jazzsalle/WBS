...
| D-7 | **컬럼은 위치가 아니라 헤더 텍스트로 매핑한다.** 인건비 열 위치가 부처마다 다르다. 매핑 결과는 미리보기에 드러내 사용자가 고칠 수 있어야 한다. |
...
| D-17 | 반영 전 **`import_snapshots`에 스냅샷**을 남긴다(I-17과 같은 트랜잭션). 산출근거 임포트는 행을 지울 수 있으므로(D-15 교체) 되돌릴 길이 반드시 있어야 한다. 스냅샷은 `budget_items` 계획액뿐 아니라 **삭제되는 `budget_details` 행 전체**를 담는다. |
...
parseDetailRows(sheet, block)      → D-5~D-10 행 → 산출근거 초안
```
...
export const SUBCATEGORY_ALIASES: Record<BudgetCategory, Record<string, string>> = {
  ...
  // 나머지 비목은 부록 A.5 라벨의 정규화형으로 충분하다 (실측 2종 기준)
};
```
