...
export interface RawCell {
  ...
  errorText?: string;
  /**
   * D-22: 엑셀 **백분율 서식** 셀인가. 어댑터가 표시 서식을 보고 채운다.
   *
   * `value`는 저장된 원본 그대로다 — 화면에 `10.0%`로 보이는 셀의 값은 `0.1`이다.
   * 값을 어댑터가 바꾸면 `RawCell.value`가 원본과 달라져 다른 규칙이 전부 흔들리므로,
   * **되돌리는 것(×100)은 그 값을 인자로 읽는 파서의 몫**이다 (§6.11.3 D-22).
   *
   * 선택 필드다: 총괄표 경로(§6.8)는 금액만 읽으므로 채우지 않아도 동작이 같다.
   */
