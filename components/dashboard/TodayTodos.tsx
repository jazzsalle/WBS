  ...
  return (
    ...
    >
      <h2 id="dashboard-todos-title" className="text-base font-bold text-grey-900">
        오늘의 To-Do
        <span className="ml-2 text-xs font-normal text-grey-500">오늘 마감 + 지난 마감</span>
      </h2>
      ...
      {items.length === 0 ? (
        ...
        <ul className="mt-4 space-y-2">
          {items.map((item) => (
            <li key={item.id} className="flex flex-wrap items-center gap-2 rounded-xl bg-grey-50/60 px-3 py-2">
              <Badge tone={PRIORITY_TONES[item.priority]}>{PRIORITY_LABELS[item.priority]}</Badge>
              <span className="min-w-0 flex-1 truncate text-sm text-grey-900" title={item.title}>
                {item.title}
              </span>
              {item.projectId !== null && (
                <Link
                  href={`/projects/${item.projectId}`}
                  className="max-w-[10rem] truncate text-xs text-grey-500 hover:text-blue-600 hover:underline"
                >
                  {item.projectName ?? '(삭제된 과제)'}
                </Link>
              )}
              <span className="text-xs tabular-nums text-grey-500">{item.dueDate}</span>
      ...
      <div className="mt-3 text-right">
        <Link href="/todos" className="text-xs text-grey-500 hover:text-blue-600 hover:underline">
          To-Do 전체 보기 →
        </Link>
      </div>
    </section>
