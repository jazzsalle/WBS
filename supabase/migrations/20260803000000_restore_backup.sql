-- =============================================================================
-- restore_backup (schema_version = 1 유지 — 함수 추가는 앱 계약을 바꾸지 않는다)
-- SOT §8.7 K-5·K-7·K-8, §8.3 SA-3, §14.3 RLS-1
--
--  - K-5 : payload.schemaVersion ≠ app_settings.schema_version 이면 거부
--  - K-7 : 단일 트랜잭션에서 FK 의존 역순 전 행 DELETE → 정순 INSERT (id 보존).
--          부분 복원·병합 없음 — 전체 대체만
--  - K-8 : app_users는 삭제·삽입 모두 제외, app_settings는 행 삭제 없이
--          schema_version(·상수 id) 제외 컬럼만 UPDATE
--
-- security definer 인 이유 (X-2의 추가 예외):
--  - 전체 대체 복원은 RLS·보호 트리거 위에서 도는 관리 작업이다. 호출자 검증은
--    approve_user와 같은 패턴(app_users.active=true 자기검증)으로 함수 안에서 직접 한다.
--  - definer(소유자 postgres)에서는 current_user ≠ 'authenticated' 라서
--    app_settings_guard/app_users_guard 트리거를 통과한다 — 단 schema_version과
--    app_users는 애초에 건드리지 않는다.
--  - anon/public 실행 권한은 revoke하고 authenticated에만 grant한다 (RLS-1의 취지).
-- =============================================================================
...
begin
  ...
  for i in reverse array_length(c_tables, 1) .. 1 loop
    execute format('delete from public.%I', c_tables[i]);
  end loop;
...
-- 실행 권한: 함수 생성 시 붙는 PUBLIC 기본 grant를 걷어내고 authenticated만 남긴다
